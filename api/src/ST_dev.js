...
$.extend(ST, /** @lends ST# */{
  ...
  userIsLogined: false,
    /**
     * 调试开关，ST_dev.js中为true，ST.js中为false
     * 此标识主要是用来区分开发版本和正式发布版本，在开发过程中比较有用
     * @type bool
     * @default true
     * @example
     * //判断是否为开发版本，来确定请求数据的url是指向测试模拟数据url还是正式url
     * var ajaxUrl = ST.debug ? "./mock.json" : "./getData.do";
     */
  debug: true,
    /**
     * 验证码组
  ...
  include: [],
    /**
     * 预加载的模板列表，在ST.todoList方法执行之前调用{@link ST#getTmp|ST.getTmp}预加载完毕
     * @type string[]
     * @example
     * $.extend(ST,{
     *      jsTemplates:["common"],//预加载指定目录中的common.html文件
     *      todoList:function() {
     *          //在执行之前，jsTemplates中的模板已加载完毕
     *          var dom = {@link ST.JTE}.fetch("card_tip").toFilled();
     *      }
     * });
     *
     * ST.init();
  ...
  jsTemplates: [],
    /**
     * 需要添加的行为列表，ST组件所有资源加载完成后，执行此方法中的内容
     */
  todoList: function () {
  },
    /**
     * 需要包含的模板数组 用于确认JS模板加载完成,参见{@link ST#init|ST.init}方法源码
     * @type string[]
     * @private
     */
  jtCount: [],
  ...
  debugErro: function (e, xhr, opt) {
     ...
     * 计算位置
     * @func
     * @param {jQueryObject} em     要计算的dom元素
     * @param {jQueryObject} em1
     * @param pos
     * @param align
     * @returns {{x: *, y: *, l: *, t: *, w: *, h: *, dir: number}}
     * @private
     */
  _posCalculate: function (em, em1, pos, align) {
    ...
    /**
     * ajax获取指定名称的模板
     * @param   {String}    o.v     模板地址，实际url地址为{@link ST.PATH.JSTMP}+o.v+".html"
     * @param   {Function}  o.cb    加载完成后的回调函数
     * @example
     * ST.getTmp({v:"test"});
     */
  getTmp: function (o) {
   ...
   */
    /**
     * 统一的方法入口
     * @param   {Function} fn       要执行的方法名
     * @param   {Object[]} [pars]   传递的方法参数
     * @returns {*}
     * @private
     * @example
     * ST.todo("goto","a","b"); //执行了goto方法 参数为 a ,b;
     */
  ...
  _ProcessFun: function (func) {
    ...
    /**
     * 延迟加载器,延迟加载指定的区域,延迟加载的对象必须具备指定的高宽
     * @param   {Object}    o
     * @param   {string}    o.targ=img  延迟加载的标签
     * @param   {string}    o.attr      延迟属性
     */
  lazyLoader: function (a) {
    ...
    /**
     * 快速构造一个tipMsg
     * @param   {string|object}     msg     提示提示信息
     * @param   {number}    hideDelay       自动消失的时间（单位毫秒）
     * @param   {boolean}   mask            是否显示遮罩
     * @example
     * ST.tipMsg("弹出提示",3000,1); //弹出一个具备背景遮罩的"弹出提示框"3秒后消失隐藏
     * @returns {$.Widget.Tip|*|ST.$tip}
     */
  ...
  hideMsg: function (flag) {
    ...
    /**
     * 快速构造一个noticeMsg
     * @param   {obejct}    ops   详见noticebox show方法参数说明
     * @returns {$.Widget.Noticebox|*|ST.$notice}
     * @example
  ...
  /**
   * 快速构造一个msgbox
   * @param {object}    ops msgbox的参数选择,详见$.Widget.Msgbox
   * @param {string|HTMLString}     ops.title    标题
   * @param {string|HTMLString|DOMElement}  ops.content 内容
   * @param {string}    ops.url     链接
   * @param {boolean}   ops.havaSpace   是否具有空白
   * @param {boolean}   ops.hideBottom  隐藏底部按钮
   * @param {object[]}  btns    按钮数组 数组内容为对象 {text:"确定",fun:function(){}}
   * @param {number}    w       宽度
   * @param {number}    h       高度
   * @param {boolean}   mk      遮罩(默认false)
   * example
   * //一个链接到百度的弹出框
  ...
  msgbox: function (ops, btns, w, h, mk) {
     ...
     * @param   o.data
     * @param   o.template
     * @param   {function}  o.selFn 选择事件
     * @param   {number}   o.pos        默认3
     * @param   {number}   o.align
     * @param   {string}   o.trigger    触发事件，默认click
     *
     *
     * @returns {$.Widget.DropContainer}
     */
  dc: function (o) {
     ...
     * @param o.id
     * @param o.data
     * @param   {string} [o.template=common_tooltip]
     * @param   {string} [o.trigger=mosuseover]
     * @param   o.pos
     * @param   o.align
     * @param   o.hasdir
     *
  ...
  confirm: function (c, tle, okfn, clfn, w, h) {
    ...
    /**
     * 快速构造一个下拉框
     * @param id    {string}    用于构造下拉框的ID
     * @param data  {string[]}    用于构造下拉框的数据
     * @param selFn {function}  选中后的回调
     * @param h {number}    下拉显示的数量   默认为6
     * @example
  ...
  ddList: function (id, data, selFn, h) {
    ...
    /**
     * 是否存在制定的对象
     * @param n {string}    对象名@eg   ST.Verify
     * @example
     * ST.Verify
  ...
  hasObject: function (n) {
    ...
    /**
     * 获取JS对象 仅会加载一次JS对象
     * @param n {string}    对象名* @ps   这里文件名同对象名
     * @param succ  {function}
     * @param fail  {function}
     */
    ...
    /**
     * 序列化表单,查找所有具备name的节点信息
     * @param f {jQueryObject|Form}
     * @example
     * ST.serObj("#form1"); //序列化表单form1  返回序列化后的Object
     * @returns {{}}
     */
  serObj: function (f) {
    var a = {};
    ...
    /**
     * 序列化表单,查找所有具备name的节点信息
     * @param f {jQueryObject|selector}
     * @param s {bool}  是否序列化为对象
     * @example
     * //序列化表单form1
     * ST.serForm("#form1");
     * //or
     * var $("#form1").serialize(); //jquery方法
     * @returns {Array}
     */
   ...
   */
     ...
     * ajax提交表单
     * @param o
     * @param o.f   {jQueryObject|selector} 表单对象或者 "#form"
     * @param o.succ    {function}  成功后的回调 return j(json对象)
     * @param o.error   {function}  成功后的回调 return e(错误对象)
     * @returns {boolean}
     * @example
ST.postForm({
    f:"#Form1",
  ...
  reload: function (time, url) {
    ...
    /**
     * 限制字符输入
     * @param cid   {string}    限制的输入控件
     * @param lmt   {number}    限制的数字
     * @param sid   {string}    文字显示ID
     * @param flag
     * @example
     * 见 limitLn
     */
  ...
  logOut: function () {
     ...
     * 滚动到制定区域
     * @param el    滚动到的元素
     * @param s     滚动时间 //默认800号码 小于50毫秒内则使用scrollIntoView
     * @returns {boolean}
     */
  ...
  JTE: (function () {
    ...
    return /**@lends ST.JTE#*/{
        /**
         * 指定ST.JTE使用的模板
         * @param a 所有模板所在的dom id，后续所有的模板都将从此id内的字符串中解析出来
         * @param c 指定的模板id，如果指定，ST.JTE会缓存该id的模板
         * @returns {ST}
         *
         * @example
         * <div id="tmpl">
         ...
         *
         * //将#tmpl作为模板容器
         * ST.JTE.use('tmpl',null)
         *
         * //将#tmpl作为模板容器，同时缓存id为alert的模板
         * ST.JTE.use('tmpl','alert')
         */
      using: function (a, c) {
        w = ($("#" + a).size() != 0) ? $("#" + a).html() : a;
        w = (w + '').replace(/\s+/g, ' ');
      ...
      getFilled: function (a) {
        ...
        /**
         * 生成模板，填充指定dom
         * @param a {jQueryObject|String[]} jQuery对象或者id集合}
         * @param b {object} 数据对象
         * @param c {@link ST.JTE}对象,当被传入时，方法返回此对象
         * @returns {ST}
         */
...
$.extend(
  ...
  isMouseLeaveOrEnter: function (e, handler) {
     ...
     * 获取文档大小
     * @param d
     * @returns {{fullWidth: number, fullHeight: number, viewWidth: number, viewHeight: *, scrollLeft: *, scrollTop: *, scrollWidth: *, scrollHeight: *, clientLeft: number, clientTop: number}}
     * @example
     * $.documentSize();
     */
...
$.extend({
  ...
  Widget: {
        /**
         * @namespace $.Widget.DropContainer
         */
    DropContainer: $.createClass({}, /** @lends $.Widget.DropContainer# **/{
        /**
         * 是否展示
         */
      isShown: false,
      ...
      init: function (id, trigger) {
         ...
         */
    Control: $.createClass({}, /** @lends $.Widget.Control# */{
        /** 属性 */
      isShown: false,
    /**初始化*/
...
$.Widget.Tip = $.createClass($.Widget.Control,
    function (_base) {

    /**
     * @namespace $.Widget.Tip
     * @extends $.Widget.Control
     *
     */

/**
 * 配置文件
 * @prop $.Widget.Tip~config
 * @inner
 * @type {{temp: string}}
 */
  var config = {
            /**
             * 模板
             * @defaultvalue common_tip
             */
    temp: "common_tip"
  }
  return /**@lends $.Widget.Tip#*/{
      /**初始化*/
    init: function (ops) {
      var t = this;
    ...
    setMsg: function (msg) {
      ...
      /**
       *
       * @param msg {object} 参见{@link $.Widget.Tip#setMsg}
       * @param useMask {bool} 是否需要遮罩效果
       * @param zIdx 参见{@link $.Widget.Control#setZIdx}
       */
    show: function (msg, useMask, zIdx) {
      ...
      /**
       * @param flag 参见{@link $.Widget.Control#hide}
       */
    hide: function (flag) {
      var t = this;
...
$.Widget.Msgbox = $.createClass($.Widget.Control, function (_base) {
  return /**@lends $.Widget.Msgbox#*/{
      /**
       * @defaultvalue common_msgbox
       * */
...
/**
 * Notice class 提示消息
 * @namespace $.Widget.Noticebox
 * @prop $count {number} 默认值6000
 */
$.Widget.Noticebox = $.createClass($.Widget.Control, function (_base) {
    return /**@lends $.Widget.Noticebox#*/{
        /**
         * 模板
         * @defaultvalue common_noticebox
         * */
         ...
         *
         * @param ops
         * @param ops.mask {bool} 是否启用遮罩
         * @param ops.hideDelay {number} 自动关闭延迟 毫秒
         * @param ops.width {string} 弹窗宽度
         * @param ops.height {string} 提示框高度
         * @param ops.title {string} 提示框标题
         * @param ops.type {string} 提示框类型
         * @param ops.content {string} 提示框内容
         */
         show: function (ops) {
            var t = this;
            if (!t.isShown) {
...
/**
 * @namespace $.Widget.DropDownList
 */
$.Widget.DropDownList = $.createClass({}, function () {
  return/**@lends $.Widget.DropDownList#*/{
      /**@inner*/
    showNumber: 6,
      /**@inner*/
    itemHeight: 30,
...
//便捷方法
$.extend(String.prototype, /**@lends String.*/{
    /**参见{@link String.contains}*/
  c: String.prototype.contains,
    /**参见{@link String.escape}*/
  e: String.prototype.escape,
    /**参见{@link String.encodeURI}*/
  i: String.prototype.encodeURI,
    /**参见{@link String.encodeURIComponent}*/
  u: String.prototype.encodeURIComponent,
    /**参见{@link String.format}*/
  f: String.prototype.format,
    /**参见{@link String.trim}*/
  t: String.prototype.trim,
    /**参见{@link String.byteLen}*/
