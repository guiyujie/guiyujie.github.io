    ...
    function (_base) {
        return {
            ...
            setup: function () {
                ...
                a.$panel = $("#PCAL_PANEL_" + a.cid).bind("click",
                    function(e) {
                        var em = e.target;
            ...
            _setupYm: function () {
                ...
                if (a.monthPicker == "ddlist") {
                    ...
                    var dp = $("#PCAL_" + a.cid);
                    a._ym = $("#PCAL_PANEL_" + a.cid).attr({
                        readOnly: true
                    }).bind("click",
                        function () {
                            if (!a.YMPikcer) {
                                a.YMPikcer = new ST.YMPicker({
                                    id: document.getElementById("PCAL_" + a.cid),
                                    year: a.curYear,
                                    month: a.curMonth,
                                    day: a.curDay
                                });
                                a.YMPikcer.onShow =function(){
                                    //移除事件绑定
                                ...
                                a.YMPikcer.onSelected = function (arg) {
                                    ...
                                    a.curDate = a.curYear  + "-" + (a.curMonth < 10 ? '0' + a.curMonth : a.curMonth) + "-" + (a.curDay < 10 ? '0' + a.curDay : a.curDay);
                                    a.showDays();
                                    //a.show();
                                    //事件绑定
            ...
            init: function (a, b, c, d) {
                ...
                g.__minDate = g.minDate;
                g.maxDate = g.endYear + "-12-31";
                g.__maxDate = g.maxDate;
                g.today = today.join("-");
            ...
            show: function () {
                if (this.isShown) {
                    ...
                    if(a.timeChoose){
                        //时间选取
                        if(!a.$timerChoose){
                            var tcid= 'PCAL_timeChoose_'+a.cid;
                            a.$timerChoose = $("#"+tcid).evProx({
                                click: {
                                    'li': function (e) {
            ...
            showTimeChoosed:function(){
                var a=this;
                var tcid= 'PCAL_timeChoose_'+a.cid;
                var tc = [a.curHour, a.curMinute].join(":");
                ST.JTE.fetch('common_timechoose_temp').toFill(tcid,{timechoosed:tc})
            },
            fill: function (e) {
                var t = this, a;
                a = e.target;
            ...
            //选择日期
            _selectDate: function (date,time) {
                var t = this, y, m, d, dd, tmp;
                dd = ($.Lang.Browser.isIE && (tmp = date.toString().split("-")).length > 1) ? new Date(tmp[0], tmp[1] - 1, time?tmp[2].split(" ")[0]:tmp[2]) : new Date(date);
            ...
            _setTime:function(ti){
                ...
                if (t.timePicker) {
                    ...
                    if(ti){
                        ...
                        t.$timePicker && t.$timePicker.setValue(new Date(d + ' ' + timeString));
                    }else if(t.$timePicker){
                    ...
                    t.curSecond = time['S'];
                    d += ' ' + timeString;
                }else if(t.timeChoose){
                    d += ' ' + [t.curHour,t.curMinute].join(":");
                }
                t.Jid.val(d);
                t.curDateTime  = d;
                var c = {date: d};
