          ...
          ST.postForm({
            ...
            succ: function (j) {
              if (a = $(f).attr('afterSubFun')) {
                if (!$(f).attr('noDftMsg')) $ST.tipMsg(j.info || j.message || '提交成功', 1000);
                ST.todo(a, j);
              } else {
                if (!$(f).attr('noDftMsg')) $ST.tipMsg(j.info || j.message || '提交成功', 5000, !0);
                j.data && window.setTimeout(function () {
                  if (j.data.url) {
                    parent.location.href = j.data.url;
                  } else {
                    $ST.reload();
                  }
                }, 1000);
            ...
            error: function (e) {
              if (!$(f).attr('noDftMsg')) $ST.tipMsg(e.info || e.message || '提交失败', 1000);
